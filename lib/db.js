...
exports.sendBroadcast = function (studentId, courseId, message, callback) {
  ...
  bcst.save(function (err) {
    if (err) return callback(err, null);
    Broadcast
      .findOne({'_id' : bcst._id})
      .populate('from')
      .exec(callback);
  });
}
...
/*
 * DB wrapper for removing a post
 */
exports.removePost = function (studentId, postId, callback) {
  Broadcast.remove({'_id': postId, 'from': studentId}, callback);
}
 ...
 * DB wrapper for getting a user's feed.
 */
exports.getCourseFeed = function (courseId, callback) {
  Broadcast
